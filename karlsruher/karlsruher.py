...
class Karlsruher:
    ...
    def __init__(self, config, brain=None, twitter=None):
        ...
        ## Connect Twitter
        credentials = '{}/credentials.yaml'.format(self.config.home)
  ...
  or:
    ...
    def run():
        ...
        try:
            ...
            if not home:
                home_from_env = os.environ.get('KARLSRUHER_HOME')
                if home_from_env and str(home_from_env) != 'KARLSRUHER_HOME':
                    print('Using KARLSRUHER_HOME:', home_from_env)
                    home = home_from_env
